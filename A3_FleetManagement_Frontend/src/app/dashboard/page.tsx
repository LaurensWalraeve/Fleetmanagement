...
export default function DashboardPage() {
  ...
  const [totalFuelCards, setTotalFuelCards] = useState(null);

  // Function to fetch data from the C# endpoints
  const fetchData = async () => {
    try {
      const responseVehicles = await axios.get(
        "http://localhost:54315/api/Vehicle/Count"
      );
      const responseDrivers = await axios.get(
        "http://localhost:54315/api/Driver/Count"
      );
      const responseFuelCards = await axios.get(
        "http://localhost:54315/api/FuelCard/Count"
      );

      setTotalVehicles(responseVehicles.data);
      setTotalDrivers(responseDrivers.data);
      setTotalFuelCards(responseFuelCards.data);
    } catch (error) {
